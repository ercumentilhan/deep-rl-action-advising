...
class ReplayBuffer(object):
    ...
    def add(self, transition):  # obs_t, action, reward, obs_tp1, done, *args, **kwargs)
        ...
        # Determine _next_idx to be overwritten by checking if there are any preserve flags
        find_next = True
        while find_next:
            self._next_idx = int((self._next_idx + 1) % self._maxsize)
            # Check if the target destination is preserved
            # self._storage[self._next_idx][-1]: 'preserve'
            if self._next_idx < len(self._storage) and self._storage[self._next_idx][-1]:
                find_next = True
            else:
                find_next = False
