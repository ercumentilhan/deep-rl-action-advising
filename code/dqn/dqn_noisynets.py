...
class NoisyNetsDQN(DQN):
    def __init__(self, id, config, session, stats, demonstrations_datasets, network_naming_structure_v1 = False):
        ...
        self.tf_vars['evaluation'] = tf.compat.v1.placeholder(tf.compat.v1.bool, name=self.name_online + '_EVALUATION')
        self.tf_vars['evaluation_tar'] = tf.compat.v1.placeholder(tf.compat.v1.bool, name=self.name_target + '_EVALUATION')
    ...
    def get_action(self, obs):
        # TODO: Disable this to speed-up execution (does double forward pass)
        q_vals_exploration = self.get_q_values(obs, evaluation=False)[0]  # Exploration involved
        q_vals_exploitation = self.get_q_values(obs, evaluation=True)[0]  # Exploitation only

        action_exploration = np.argmax(q_vals_exploration)
        action_exploitation = np.argmax(q_vals_exploitation)

        return action_exploration, action_exploration != action_exploitation
